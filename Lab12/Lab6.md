# Lab 6: Building a Flask Customer Health Dashboard Using GitHub Copilot
...
1. Create a new project folder named **Lab06** in your `C:\` drive and open it in Visual Studio Code. Open **Terminal → New Terminal** and click the dropdown to select **Git Bash**. Run the below commands to set up the project:
...
1. Create a new file: **data.py** in the `customer-health-dashboard` root folder.

1. Type the following **intent-driven comments** at the top of the file and press **Enter** after each line:
...
1. **Pause and observe.** Copilot offers dimmed ghost text suggestions as you type — sometimes completing the current line, sometimes suggesting a whole block of code.
...
1. **Do NOT blindly accept.** Verify the following:
...
1. If Copilot's output is incomplete or has inconsistencies, **refine using Copilot inline chat**. Press **Ctrl+I** and type:
...
1. Create the `templates/` folder in the `customer-health-dashboard` root.
...
1. Review the Copilot Chat output carefully before accepting it. Click **Keep** if it matches your requirements:
...
1. Open the VS Code **integrated terminal** (**Ctrl+`** or **Terminal → New Terminal**) and switch to **Git Bash**. Ensure your virtual environment is still activated (you should see `(venv)` in the prompt), then run:

   ```
   python app.py
   ```

1. You should see Flask startup output similar to:
...
1. **If errors occur**, use Copilot to debug. Select the error message in the terminal, press **Ctrl+I**, and type:
